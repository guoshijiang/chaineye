...
@check_admin_login
def back_course_article_check(request, caid):
    b_course = CourseArtcle.objects.filter(id=caid).first()
    b_course.is_active = True
    b_course.save()
    return redirect('back_course_artcle', b_course.course.id)
...
@check_admin_login
def course_comment_check(request, cid):
    cc = CourseCommet.objects.filter(id=cid).first()
    cc.is_active = True
    cc.save()
    return redirect('back_course_comment', cc.artcle.id)